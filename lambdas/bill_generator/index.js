...
exports.handler = async (event) => {
  ...
  // Build PDF into an in-memory buffer
  const pdfBuffer = await new Promise((resolve, reject) => {
    try {
      ...
      doc.font('Helvetica')
        .text(`Consumer Number: ${consumerNumber}`, 320, 180)
      ...
      // Footer
      doc.strokeColor('#E0E0E0').lineWidth(1).moveTo(50, 745).lineTo(545, 745).stroke();
      doc.fillColor('#777777').fontSize(8).font('Helvetica');
      doc.text('Generated by SmartGrid Utility Management Platform', 50, 755, { align: 'center', width: 495 });
      doc.text(`Generation Timestamp: ${new Date().toLocaleString()}`, 50, 767, { align: 'center', width: 495 });
  ...
  console.log(`Successfully uploaded to S3: ${BUCKET_NAME}/${fileName}`);
  return { amount, s3_key: fileName };
};
